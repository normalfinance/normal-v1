...
impl User {
    ...
    pub fn get_position_mut(&mut self, market_index: u16) -> NormalResult<&mut Position> {
        Ok(&mut self.positions[get_position_index(&self.positions, market_index)?])
    }

    pub fn force_get_position_mut(&mut self, market_index: u16) -> NormalResult<&mut Position> {
        let position_index = get_position_index(&self.positions, market_index).or_else(|_|
            add_new_position(&mut self.positions, market_index)
        )?;
    ...
    pub fn get_order_index_by_user_order_id(&self, user_order_id: u8) -> NormalResult<usize> {
        self.orders
            .iter()
            .position(|order| {
    ...
    pub fn update_advanced_lp_status(&mut self, advanced_lp: bool) -> NormalResult {
        if advanced_lp {
...
impl Position {
    ...
    pub fn simulate_settled_lp_position(
        ...
        let mut new_remainder_base_asset_amount = settled_position.remainder_base_asset_amount
            .cast::<i64>()?
    ...
    pub fn worst_case_base_asset_amount(&self, oracle_price: i64) -> NormalResult<i128> {
        self.worst_case_liability_value(oracle_price).map(|v| v.0)
    }

    pub fn worst_case_liability_value(&self, oracle_price: i64) -> NormalResult<(i128, u128)> {
    ...
    pub fn get_direction(&self) -> PositionDirection {
        if self.base_asset_amount >= 0 { PositionDirection::Long } else { PositionDirection::Short }
    }

    pub fn get_direction_to_close(&self) -> PositionDirection {
        if self.base_asset_amount >= 0 { PositionDirection::Short } else { PositionDirection::Long }
    }
...
impl OrderFillSimulation {
    pub fn riskier_side(ask: Self, bid: Self) -> Self {
        if ask.free_collateral_contribution <= bid.free_collateral_contribution { ask } else { bid }
    }
...
impl Position {
    ...
    pub fn get_token_amount(&self, spot_market: &SpotMarket) -> NormalResult<u128> {
        get_token_amount(self.scaled_balance.cast()?, spot_market)
    }

    pub fn get_signed_token_amount(&self, spot_market: &SpotMarket) -> NormalResult<i128> {
        get_signed_token_amount(get_token_amount(self.scaled_balance.cast()?, spot_market)?)
    }
...
impl Order {
    ...
    pub fn force_get_limit_price(
        ...
        match self.get_limit_price(valid_oracle_price, fallback_price, slot, tick_size)? {
            ...
            None => {
                ...
                msg!("Could not get limit price at {}:{}", caller.file(), caller.line());
    ...
    pub fn has_limit_price(self, slot: u64) -> NormalResult<bool> {
        Ok(self.price > 0 || !is_auction_complete(self.slot, self.auction_duration, slot)?)
    }
    ...
    pub fn must_be_triggered(&self) -> bool {
        matches!(self.order_type, OrderType::TriggerMarket | OrderType::TriggerLimit)
    }
...
impl UserStats {
    pub fn update_maker_volume_30d(&mut self, quote_asset_amount: u64, now: i64) -> NormalResult {
    ...
    pub fn update_taker_volume_30d(&mut self, quote_asset_amount: u64, now: i64) -> NormalResult {
    ...
    pub fn update_filler_volume(&mut self, quote_asset_amount: u64, now: i64) -> NormalResult {
    ...
    pub fn increment_total_referrer_reward(&mut self, reward: u64, now: i64) -> NormalResult {
    ...
    pub fn increment_total_referee_discount(&mut self, discount: u64) -> NormalResult {
