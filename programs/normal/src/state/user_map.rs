...
impl<'a> UserMap<'a> {
    ...
    pub fn insert(&mut self, user: Pubkey, account_loader: AccountLoader<'a, User>) -> NormalResult {
        validate!(
...
#[cfg(test)]
impl<'a> UserMap<'a> {
    pub fn load_one<'b: 'a>(account_info: &'b AccountInfo<'a>) -> NormalResult<UserMap<'a>> {
...
impl<'a> UserStatsMap<'a> {
    ...
    #[track_caller]
    #[inline(always)]
    pub fn get_ref_mut(&self, authority: &Pubkey) -> NormalResult<RefMut<UserStats>> {
        let loader = match self.0.get(authority) {
            Some(loader) => loader,
            None => {
                let caller = Location::caller();
...
#[cfg(test)]
impl<'a> UserStatsMap<'a> {
    pub fn load_one<'b: 'a>(account_info: &'b AccountInfo<'a>) -> NormalResult<UserStatsMap<'a>> {
