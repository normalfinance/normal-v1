...
impl LiquidityPositionBundle {
	...
	fn update_bitmap(&mut self, bundle_index: u16, open: bool) -> Result<()> {
		if !LiquidityPositionBundle::is_valid_bundle_index(bundle_index) {
...
mod position_bundle_initialize_tests {
	...
	#[test]
	fn test_default() {
		let position_bundle = LiquidityPositionBundle {
			..Default::default()
		};
	...
	#[test]
	fn test_initialize() {
		let mut position_bundle = LiquidityPositionBundle {
			..Default::default()
		};
		let position_bundle_mint = Pubkey::from_str(
...
mod data_layout_tests {
	...
	fn test_position_bundle_data_layout() {
		...
		let mut position_bundle_data = [0u8; LiquidityPositionBundle::LEN];
		let mut offset = 0;
		position_bundle_data[offset..offset + 8].copy_from_slice(
			&LiquidityPositionBundle::discriminator()
		);
		offset += 8;
		position_bundle_data[offset..offset + 32].copy_from_slice(
			&position_bundle_position_bundle_mint.to_bytes()
		);
		offset += 32;
		position_bundle_data[
			offset..offset + POSITION_BITMAP_USIZE
		].copy_from_slice(&position_bundle_position_bitmap);
		offset += POSITION_BITMAP_USIZE;
		position_bundle_data[
			offset..offset + position_bundle_reserved.len()
		].copy_from_slice(&position_bundle_reserved);
		offset += position_bundle_reserved.len();
		assert_eq!(offset, LiquidityPositionBundle::LEN);

		// deserialize
		let deserialized = LiquidityPositionBundle::try_deserialize(
			&mut position_bundle_data.as_ref()
		).unwrap();
