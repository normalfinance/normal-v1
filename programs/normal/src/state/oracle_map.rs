...
impl<'a> OracleMap<'a> {
    ...
    pub fn get_account_info(&self, pubkey: &Pubkey) -> NormalResult<AccountInfo<'a>> {
        Ok(self.oracles.get(pubkey).ok_or(ErrorCode::OracleNotFound)?.account_info.clone())
    }
    ...
    pub fn validate_oracle_account_info<'c>(account_info: &'c AccountInfo<'a>) -> NormalResult {
