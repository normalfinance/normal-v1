...
use crate::ids::{
	bonk_oracle,
	bonk_pull_oracle,
	normal_oracle_receiver_program,
	pepe_oracle,
	pepe_pull_oracle,
	pyth_program,
	usdc_oracle,
...
pub const PYTH_STABLECOIN_IDS: [Pubkey; 2] = [
	usdc_oracle::id(),
	usdt_oracle::id(),
];
...
impl<'a> OracleMap<'a> {
	...
	pub fn get_price_data_and_validity(
		...
		if self.price_data.contains_key(pubkey) {
			...
			let oracle_validity = if
				let Some(oracle_validity) = self.validity.get(pubkey)
			{
				*oracle_validity
			} else {
	...
	) -> NormalResult<OracleMap<'a>> {
		let mut oracles: BTreeMap<
			Pubkey,
			AccountInfoAndOracleSource<'a>
		> = BTreeMap::new();

		while let Some(account_info) = account_info_iter.peek() {
			if account_info.owner == &pyth_program::id() {
				let account_info: &AccountInfo<'a> = account_info_iter
					.next()
					.safe_unwrap()?;
				...
				continue;
			} else if account_info.owner == &normal_oracle_receiver_program::id() {
				let account_info: &AccountInfo<'a> = account_info_iter
					.next()
					.safe_unwrap()?;
				let pubkey = account_info.key();
	...
	pub fn validate_oracle_account_info<'c>(
		account_info: &'c AccountInfo<'a>
	) -> NormalResult {
