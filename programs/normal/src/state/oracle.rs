...
impl HistoricalIndexData {
    ...
    pub fn default_with_current_oracle(oracle_price_data: OraclePriceData) -> NormalResult<Self> {
...
pub fn get_sb_on_demand_price(
    ...
    let pull_feed_account_info: Ref<PullFeedAccountData> = load_ref(price_oracle).or(
        Err(ErrorCode::UnableToLoadOracle)
    )?;
...
fn convert_switchboard_decimal(switchboard_decimal: &SwitchboardDecimal) -> NormalResult<i128> {
...
impl StrictOraclePrice {
    ...
    pub fn validate(&self) -> NormalResult {
        ...
        if let Some(twap) = self.twap_5min {
            validate!(twap > 0, ErrorCode::InvalidOracle, "oracle_price_twap={} (<= 0)", twap)?;
        }
