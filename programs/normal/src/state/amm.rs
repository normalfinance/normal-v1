...
impl AMM {
    ...
    pub fn calculate_per_lp_delta(
        ...
        let per_lp_fee: i128 = if fee_to_market > 0 {
            get_proportion_i128(fee_to_market, LP_FEE_SLICE_NUMERATOR, LP_FEE_SLICE_DENOMINATOR)?
                .safe_mul(base_unit)?
    ...
    pub fn get_target_base_asset_amount_per_lp(&self) -> NormalResult<i128> {
        ...
        let target_base_asset_amount_per_lp: i128 = if self.per_lp_base > 0 {
            ...
            self.target_base_asset_amount_per_lp.cast::<i128>()?.safe_mul(rebase_divisor)?
        } else if self.per_lp_base < 0 {
            let rebase_divisor = (10_i128).pow(self.per_lp_base.abs().cast()?);
            self.target_base_asset_amount_per_lp.cast::<i128>()?.safe_div(rebase_divisor)?
        } else {
    ...
    pub fn amm_wants_to_jit_make(&self, taker_direction: PositionDirection) -> NormalResult<bool> {
    ...
    pub fn amm_lp_allowed_to_jit_make(&self, amm_wants_to_jit_make: bool) -> NormalResult<bool> {
    ...
    pub fn reserve_price(&self) -> NormalResult<u64> {
        amm::calculate_price(self.quote_asset_reserve, self.base_asset_reserve, self.peg_multiplier)
    }
