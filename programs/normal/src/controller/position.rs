...
pub fn add_new_position(user_positions: &mut Positions, market_index: u16) -> NormalResult<usize> {
...
pub fn get_position_index(user_positions: &Positions, market_index: u16) -> NormalResult<usize> {
...
pub fn update_position_and_market(
    ...
    let (new_quote_entry_amount, new_quote_break_even_amount, pnl) = match update_type {
        PositionUpdateType::Open | PositionUpdateType::Increase => {
            ...
            let new_quote_break_even_amount = position.quote_break_even_amount.safe_add(
                delta.quote_asset_amount
            )?;
    ...
    match update_type {
        PositionUpdateType::Open | PositionUpdateType::Increase => {
            if new_base_asset_amount > 0 {
                market.amm.base_asset_amount_long = market.amm.base_asset_amount_long.safe_add(
                    new_settled_base_asset_amount.cast()?
                )?;
                market.amm.quote_entry_amount_long = market.amm.quote_entry_amount_long.safe_add(
                    delta.quote_asset_amount.cast()?
                )?;
                ...
                market.amm.base_asset_amount_short = market.amm.base_asset_amount_short.safe_add(
                    new_settled_base_asset_amount.cast()?
                )?;
                market.amm.quote_entry_amount_short = market.amm.quote_entry_amount_short.safe_add(
                    delta.quote_asset_amount.cast()?
                )?;
        ...
        PositionUpdateType::Reduce | PositionUpdateType::Close => {
            if position.base_asset_amount > 0 {
                ...
                market.amm.quote_entry_amount_long = market.amm.quote_entry_amount_long.safe_sub(
                    position.quote_entry_amount.safe_sub(new_quote_entry_amount)?.cast()?
                )?;
                market.amm.quote_break_even_amount_long =
                ...
                market.amm.quote_entry_amount_short = market.amm.quote_entry_amount_short.safe_sub(
                    position.quote_entry_amount.safe_sub(new_quote_entry_amount)?.cast()?
                )?;
                market.amm.quote_break_even_amount_short =
        ...
        PositionUpdateType::Flip => {
            if new_base_asset_amount > 0 {
                market.amm.base_asset_amount_short = market.amm.base_asset_amount_short.safe_sub(
                ...
                market.amm.base_asset_amount_long = market.amm.base_asset_amount_long.safe_sub(
                    position.base_asset_amount.cast()?
                )?;
                market.amm.base_asset_amount_short = market.amm.base_asset_amount_short.safe_add(
                    new_base_asset_amount.cast()?
...
pub fn update_lp_market_position(
    ...
    let (per_lp_delta_base, per_lp_delta_quote, per_lp_fee) = market.amm.calculate_per_lp_delta(
        delta,
        fee_to_market,
        liquidity_split,
        base_unit
    ...
    market.amm.base_asset_amount_per_lp = market.amm.base_asset_amount_per_lp.safe_add(
        -per_lp_delta_base
    )?;

    market.amm.quote_asset_amount_per_lp = market.amm.quote_asset_amount_per_lp.safe_add(
        -per_lp_delta_quote
    )?;

    // track total fee earned by lps (to attribute breakdown of IL)
    market.amm.total_fee_earned_per_lp = market.amm.total_fee_earned_per_lp.saturating_add(
        per_lp_fee.cast()?
    );
    ...
    let lp_delta_base = market.amm.calculate_lp_base_delta(per_lp_delta_base, base_unit)?;
    let lp_delta_quote = market.amm.calculate_lp_base_delta(per_lp_delta_quote, base_unit)?;
    ...
    market.amm.quote_asset_amount_with_unsettled_lp =
        market.amm.quote_asset_amount_with_unsettled_lp.safe_add(lp_delta_quote.cast()?)?;
...
pub fn update_position_with_base_asset_amount(
    ...
    market.amm.base_asset_amount_with_amm = market.amm.base_asset_amount_with_amm.safe_add(
...
pub fn increase_open_bids_and_asks(
    ...
    match direction {
        PositionDirection::Long => {
            position.open_bids = position.open_bids.safe_add(base_asset_amount_unfilled.cast()?)?;
        }
        PositionDirection::Short => {
            position.open_asks = position.open_asks.safe_sub(base_asset_amount_unfilled.cast()?)?;
        }
    }
...
pub fn decrease_open_bids_and_asks(
    ...
    match direction {
        PositionDirection::Long => {
            position.open_bids = position.open_bids.safe_sub(base_asset_amount_unfilled.cast()?)?;
        }
        PositionDirection::Short => {
            position.open_asks = position.open_asks.safe_add(base_asset_amount_unfilled.cast()?)?;
        }
    }
