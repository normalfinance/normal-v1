...
pub fn update_market_twap_stats(
	...
	let from_start = max(
		1_i64,
		SPOT_MARKET_TOKEN_TWAP_WINDOW.safe_sub(since_last)?
	);

	let collateral_token_amount = get_token_amount(
		spot_market.collateral_balance,
		spot_market,
		&SpotBalanceType::Deposit
	)?;

	let debt_token_amount = get_token_amount(
		spot_market.debt_balance,
		spot_market,
	...
	if let Some(oracle_price_data) = oracle_price_data {
		let sanitize_clamp_denominator =
			spot_market.get_sanitize_clamp_denominator()?;
		...
		spot_market.historical_oracle_data.last_oracle_price =
			oracle_price_data.price;
		spot_market.historical_oracle_data.last_oracle_conf =
			oracle_price_data.confidence;
		spot_market.historical_oracle_data.last_oracle_delay =
...
pub fn update_spot_balances(
	...
	let increase_user_existing_balance =
		update_direction == spot_balance.balance_type();
...
fn increase_spot_balance(
	...
	match balance_type {
		SpotBalanceType::Deposit => {
			spot_market.deposit_balance =
				spot_market.deposit_balance.safe_add(delta)?;
		}
		SpotBalanceType::Borrow => {
...
fn decrease_spot_balance(
	...
	match balance_type {
		SpotBalanceType::Deposit => {
			spot_market.deposit_balance =
				spot_market.deposit_balance.safe_sub(delta)?;
		}
		SpotBalanceType::Borrow => {
