...
pub fn validate_fee_structure(fee_structure: &FeeStructure) -> NormalResult {
    for (i, fee_tier) in fee_structure.fee_tiers.iter().enumerate() {
        validate_fee_tier(i, fee_tier, fee_structure.filler_reward_structure.reward_numerator)?;
    }

    let is_filler_reward_valid =
        fee_structure.filler_reward_structure.reward_numerator <= 20 &&
...
pub fn validate_fee_tier(
    ...
    let maker_rebate_valid =
        fee_tier.maker_rebate_numerator <= 30 &&
    ...
    let referee_discount_valid =
        fee_tier.referee_fee_numerator <= 20 &&
    ...
    let referrer_reward_valid =
        fee_tier.referrer_reward_numerator <= 20 &&
