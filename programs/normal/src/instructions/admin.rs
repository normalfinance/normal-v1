...
pub fn handle_update_market_amm_summary_stats(
	...
	if
		let Some(quote_asset_amount_with_unsettled_lp) =
	...
	if params.update_amm_summary_stats == Some(true) {
		let new_total_fee_minus_distributions =
			controller::amm::calculate_market_amm_summary_stats(
				market,
				oracle_price
			)?;
...
pub fn handle_update_market_max_imbalances(
	...
	msg!("updating perp market {} max imbalances", market.market_index);
...
pub fn handle_initialize_protocol_insurance_fund_shares_transfer_config(
	...
	let mut config =
		ctx.accounts.protocol_insurance_fund_shares_transfer_config.load_init()?;
...
pub fn handle_update_protocol_insurance_fund_shares_transfer_config(
	...
	let mut config =
		ctx.accounts.protocol_insurance_fund_shares_transfer_config.load_mut()?;
...
pub fn handle_initialize_index_fund(
	ctx: Context<InitializeIndexFund>,
	name: [u8; 32],
	public: bool
) -> Result<()> {
...
pub fn handle_update_index_fund_visibility(
...
pub fn handle_update_index_fund_assets(
	ctx: Context<UpdateIndexFund>,
...
pub fn handle_rebalance_index_fund(
	...
	controller::index_fund::rebalance(market, now);
...
pub struct InitializeProtocolInsuranceFundSharesTransferConfig<'info> {
	...
	pub protocol_insurance_fund_shares_transfer_config: AccountLoader<
		'info,
...
#[derive(Accounts)]
#[instruction(market_index: u16,)]
pub struct InitializeIndexFund<'info> {
	pub state: Box<Account<'info, State>>,
	#[account(
        mut,
        seeds = [b"market", market_index.to_le_bytes().as_ref()],
