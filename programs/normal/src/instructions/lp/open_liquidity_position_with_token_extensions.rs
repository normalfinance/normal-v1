...
#[derive(Accounts)]
pub struct OpenLiquidityPositionWithTokenExtensions<'info> {
	#[account(mut)]
...
/*
  Opens a new AMM Position with Mint and TokenAccount owned by Token-2022.
*/
pub fn handle_open_liquidity_position_with_token_extensions(
	...
	if with_token_metadata {
		let (name, symbol, uri) = build_position_token_metadata(
			position_mint,
			position,
			amm
		);
