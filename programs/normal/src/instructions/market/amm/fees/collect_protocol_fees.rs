...
pub struct CollectProtocolFees<'info> {
	...
	#[account(mut, constraint = token_destination_a.mint == market.amm.token_mint_synthetic)]
	pub token_destination_a: Account<'info, TokenAccount>,

	#[account(mut, constraint = token_destination_b.mint == market.amm.token_mint_quote)]
...
pub fn handle_collect_protocol_fees(
	...
	transfer_from_vault_to_owner(
		...
		market.amm.protocol_fee_owed_synthetic
	)?;
	...
	transfer_from_vault_to_owner(
		...
		&ctx.accounts.token_program,
		market.amm.protocol_fee_owed_quote
	)?;

	ctx.accounts.market.amm.reset_protocol_fees_owed();
