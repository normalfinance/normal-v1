...
pub fn calculate_bid_ask_bounds(
    ...
    let ask_bounded_base = get_proportion_u128(
        sqrt_k,
        concentration_coef,
        CONCENTRATION_PRECISION
    )?;
    ...
    let bid_bounded_base = get_proportion_u128(
        sqrt_k,
        CONCENTRATION_PRECISION,
        concentration_coef
    )?;
...
pub fn _calculate_market_open_bids_asks(
    ...
    let max_asks = if base_asset_reserve < max_base_asset_reserve {
        -max_base_asset_reserve.safe_sub(base_asset_reserve)?.cast::<i128>()?
    } else {
        0
    };
    ...
    let max_bids = if base_asset_reserve > min_base_asset_reserve {
        base_asset_reserve.safe_sub(min_base_asset_reserve)?.cast::<i128>()?
    } else {
        0
    };
...
pub fn update_mark_twap(
        ...
        amm.funding_period.safe_div(60)?.max(ONE_MINUTE.cast()?)
    {
        ...
        (
            calculate_weighted_average(
                amm.historical_oracle_data.last_oracle_price_twap.cast::<i64>()?,
                amm.last_bid_price_twap.cast()?,
                last_valid_trade_since_oracle_twap_update,
                from_start_valid
            )?,
        ...
        (amm.last_bid_price_twap.cast()?, amm.last_ask_price_twap.cast()?)
    };
...
pub fn calculate_new_oracle_price_twap(
    ...
    let (last_mark_twap, last_oracle_twap) = match twap_period {
        TwapPeriod::FundingPeriod =>
            (amm.last_mark_price_twap, amm.historical_oracle_data.last_oracle_price_twap),
        TwapPeriod::FiveMin =>
            (amm.last_mark_price_twap_5min, amm.historical_oracle_data.last_oracle_price_twap_5min),
    };
...
pub fn calculate_terminal_reserves(amm: &AMM) -> NormalResult<(u128, u128)> {
...
pub fn calculate_terminal_price_and_reserves(
    ...
    Ok((terminal_price, new_quote_asset_amount, new_base_asset_amount))
}
...
pub fn calculate_oracle_reserve_price_spread_pct(
    ...
    let (_oracle_price, price_spread) = calculate_oracle_reserve_price_spread(
        amm,
        oracle_price_data,
        Some(reserve_price)
    )?;
    ...
    price_spread
        ...
        .safe_div(reserve_price.cast::<i128>()?)
        ? // todo? better for spread logic
        .cast()
}
...
pub fn is_oracle_mark_too_divergent(
    ...
    let max_divergence = oracle_guard_rails.mark_oracle_percent_divergence.max(
...
pub fn calculate_expiry_price(
    ...
    let best_expiry_price = -amm.quote_asset_amount
