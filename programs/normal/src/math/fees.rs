...
fn calculate_taker_fee(
    ...
    if fee_adjustment < 0 {
        taker_fee = taker_fee.saturating_sub(
            taker_fee.safe_mul(fee_adjustment.unsigned_abs().cast()?)?.safe_div(FEE_ADJUSTMENT_MAX)?
        );
    } else if fee_adjustment > 0 {
        taker_fee = taker_fee.saturating_add(
            taker_fee.safe_mul(fee_adjustment.cast()?)?.safe_div_ceil(FEE_ADJUSTMENT_MAX)?
        );
    }
...
fn calculate_referee_fee_and_referrer_reward(
    ...
    let referrer_reward = match referrer_stats {
        Some(referrer_stats) => {
            let max_referrer_reward_in_epoch = referrer_reward_epoch_upper_bound.saturating_sub(
...
pub fn determine_user_fee_tier<'a>(
    ...
    determine_fee_tier(user_stats, fee_structure);
}
