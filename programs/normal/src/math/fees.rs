...
pub fn calculate_fee_for_fulfillment_with_amm(
	...
	if is_post_only {
		let maker_rebate = calculate_maker_rebate(
			quote_asset_amount,
			fee_tier,
			fee_adjustment
		)?;
		...
		let fee_to_market_for_lp = fee_to_market.safe_sub(
			quote_asset_amount_surplus
		)?;
...
fn calculate_maker_rebate(
	...
	if fee_adjustment < 0 {
		...
		maker_fee = maker_fee.saturating_add(
			maker_fee.safe_mul(fee_adjustment.cast()?)?.safe_div(FEE_ADJUSTMENT_MAX)?
		);
	}
...
fn calculate_referee_fee_and_referrer_reward(
	...
	let referrer_reward = match referrer_stats {
		Some(referrer_stats) => {
			let max_referrer_reward_in_epoch =
				referrer_reward_epoch_upper_bound.saturating_sub(
					referrer_stats.fees.current_epoch_referrer_reward
				);
...
fn calculate_filler_reward(
	...
	let min_time_filler_reward =
		filler_reward_structure.time_based_reward_lower_bound
			.safe_mul(
				multiplier
					.cast::<u128>()?
	...
	let slots_since_order = max(
		1,
		clock_slot.safe_sub(order_slot)?.cast::<u128>()?
	);
	let time_filler_reward = slots_since_order
		.safe_mul(100_000_000)
		? // 1e8
		.nth_root(4)
...
pub fn determine_user_fee_tier<'a>(
	...
	market_type: &MarketType
) -> NormalResult<&'a FeeTier> {
	return determine_fee_tier(user_stats, fee_structure);
}

fn determine_fee_tier<'a>(
	user_stats: &UserStats,
	fee_structure: &'a FeeStructure
) -> NormalResult<&'a FeeTier> {
	...
	if
		total_30d_volume >= ONE_HUNDRED_MILLION_QUOTE ||
		staked_quote_asset_amount >= TEN_THOUSAND_QUOTE - QUOTE_PRECISION_U64 ||
		staked_gov_token_amount >=
	...
	if
		total_30d_volume >= FIFTY_MILLION_QUOTE ||
	...
	if
		total_30d_volume >= TEN_MILLION_QUOTE ||
	...
	if
		total_30d_volume >= FIVE_MILLION_QUOTE ||
	...
	if
		total_30d_volume >= ONE_MILLION_QUOTE ||
		staked_quote_asset_amount >= ONE_THOUSAND_QUOTE / 2 - QUOTE_PRECISION_U64 ||
