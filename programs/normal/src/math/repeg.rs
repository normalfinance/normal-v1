...
pub fn calculate_repeg_validity_from_oracle_account(
...
) -> NormalResult<(bool, bool, bool, bool)> {
    ...
    Ok((oracle_is_valid, direction_valid, profitability_valid, price_impact_valid))
}
...
pub fn adjust_peg_cost(market: &Market, new_peg_candidate: u128) -> NormalResult<(Market, i128)> {
...
pub fn calculate_repeg_cost(amm: &AMM, new_peg: u128) -> NormalResult<i128> {
    amm.quote_asset_reserve
        .cast::<i128>()?
        .safe_sub(amm.terminal_quote_asset_reserve.cast()?)?
        .safe_mul(new_peg.cast::<i128>()?.safe_sub(amm.peg_multiplier.cast()?)?)?
...
pub fn adjust_amm(
    ...
    let delta_peg = optimal_peg.cast::<i128>()?.safe_sub(market.amm.peg_multiplier.cast()?)?; // PEG_PRECISION
    ...
    if per_peg_cost != 0 {
        budget_delta_peg = budget_i128.safe_mul(PEG_PRECISION_I128)?.safe_div(per_peg_cost)?; // PEG_PRECISION
        budget_delta_peg_magnitude = budget_delta_peg.unsigned_abs();
    }

    let use_optimal_peg =
        per_peg_cost == 0 || // if per peg cost is 0 => free
    ...
    if use_optimal_peg {
        ...
        let adjustment_cost: i128 = if adjust_k && can_lower_k {
            ...
            let new_sqrt_k = market.amm.sqrt_k
            ...
            let adjustment_cost = cp_curve::adjust_k_cost_and_update(
                &mut market_clone,
                &update_k_result
            )?;
        ...
        new_peg = if budget_delta_peg > 0 {
            market.amm.peg_multiplier.safe_add(budget_delta_peg_magnitude).unwrap_or(u128::MAX)
        } else if market.amm.peg_multiplier > budget_delta_peg_magnitude {
            market.amm.peg_multiplier.safe_sub(budget_delta_peg_magnitude)?
        } else {
            1
        };
...
pub fn calculate_optimal_peg_and_budget(
    ...
    if fee_budget < max(0, optimal_peg_cost).cast()? {
        ...
        let target_price_gap = reserve_price_before.cast::<i64>()?.safe_sub(target_price_i64)?;

        if target_price_gap.abs() > half_max_price_spread {
            let mark_adj = target_price_gap.abs().safe_sub(half_max_price_spread)?.cast()?;
...
pub fn calculate_fee_pool(market: &Market) -> NormalResult<u128> {
    ...
    let fee_pool = if
        market.amm.total_fee_minus_distributions > total_fee_minus_distributions_lower_bound
    {
