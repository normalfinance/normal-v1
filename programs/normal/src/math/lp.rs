...
use crate::{
    validate, PRICE_PRECISION, PRICE_TIMES_AMM_TO_QUOTE_PRECISION_RATIO,
};
...
pub fn calculate_settle_lp_metrics(amm: &AMM, position: &Position) -> NormalResult<LPMetrics> {
...
pub fn calculate_lp_shares_to_burn_for_risk_reduction(
    ...
    let settled_lp_position = position.simulate_settled_lp_position(market, oracle_price)?;

    let worse_case_base_asset_amount =
        settled_lp_position.worst_case_base_asset_amount(oracle_price)?;

    let open_orders_from_lp_shares = if worse_case_base_asset_amount >= 0 {
        worse_case_base_asset_amount.safe_sub(
            settled_lp_position
                .base_asset_amount
                .safe_add(position.open_bids)?
                .cast()?,
        )?
    } else {
        worse_case_base_asset_amount.safe_sub(
