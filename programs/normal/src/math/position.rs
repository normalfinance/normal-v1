...
pub fn calculate_base_asset_value(base_asset_amount: i128, amm: &AMM) -> NormalResult<u128> {
    if base_asset_amount == 0 {
        return Ok(0);
    }
...
pub fn calculate_base_asset_value_with_oracle_price(
    ...
    let oracle_price = if oracle_price > 0 { oracle_price.unsigned_abs() } else { 0 };
...
pub fn calculate_base_asset_value_and_pnl_with_oracle_price(
    ...
    let oracle_price = if oracle_price > 0 { oracle_price.abs() } else { 0 };

    let base_asset_value = market_position.base_asset_amount
        .cast::<i128>()?
        .safe_mul(oracle_price.cast()?)?
...
pub fn swap_direction_to_close_position(base_asset_amount: i128) -> SwapDirection {
    if base_asset_amount >= 0 { SwapDirection::Add } else { SwapDirection::Remove }
}
...
pub fn get_position_update_type(
    ...
    let position_base_with_remainder = if position.remainder_base_asset_amount != 0 {
        position.base_asset_amount.safe_add(position.remainder_base_asset_amount.cast::<i64>()?)?
    } else {
...
pub fn get_new_position_amounts(
    ...
    let new_quote_asset_amount = position.quote_asset_amount.safe_add(delta.quote_asset_amount)?;

    let mut new_base_asset_amount = position.base_asset_amount.safe_add(delta.base_asset_amount)?;
