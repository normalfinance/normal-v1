...
pub fn calculate_base_asset_amount_to_cover_margin_shortage(
	...
	let margin_ratio = margin_ratio.safe_mul(
		LIQUIDATION_FEE_TO_MARGIN_PRECISION_RATIO
	)?;
...
pub fn calculate_liability_transfer_to_cover_margin_shortage(
	...
	let liability_weight_component = liability_weight
		.cast::<u128>()?
...
pub fn calculate_liability_transfer_implied_by_asset_amount(
	...
	let (numerator_scale, denominator_scale) = if
		liability_decimals > asset_decimals
	{
...
pub fn calculate_asset_transfer_for_liability_transfer(
	...
	let (numerator_scale, denominator_scale) = if
		asset_decimals > liability_decimals
	{
	...
	let (asset_value_numerator_scale, asset_value_denominator_scale) = if
		asset_decimals > 6
	{
...
pub fn calculate_liquidation_multiplier(
	...
	match multiplier_type {
		LiquidationMultiplierType::Premium =>
			LIQUIDATION_FEE_PRECISION.safe_add(liquidation_fee),
...
pub fn calculate_max_pct_to_liquidate(
	...
	let total_margin_shortage = margin_shortage.safe_add(
		user.liquidation_margin_freed.cast()?
	)?;
