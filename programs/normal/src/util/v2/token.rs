...
pub fn mint_synthetic_to_vault_v2<'info>(
	...
	mint_synthetic_token_v2(amm, mint, token_vault, token_program)?;
	Ok(())
}

fn mint_synthetic_token_v2<'info>(
...
pub fn burn_synthetic_from_vault_v2<'info>(
...
pub fn transfer_from_vault_to_owner_v2<'info>(
	...
	let mut account_infos = vec![
		...
		amm.to_account_info() // authority (pool)
	];
...
fn get_transfer_hook_program_id(
	...
	for extension in extensions {
		match extension {
			...
			extension::ExtensionType::ConfidentialTransferMint => {
				...
				//
				// AMMProgram invokes TransferChecked instruction and it supports non-confidential transfer only.
				//
				...
				// Note: Only the owner (AMM account) can call ConfidentialTransferInstruction::ConfigureAccount.
			}
