...
export function groupL2(
	...
	return {
		bids: groupL2Levels(l2.bids, grouping, OrderSide.BUY, depth),
		asks: groupL2Levels(l2.asks, grouping, OrderSide.SELL, depth),
		slot: l2.slot,
	};
}
...
function groupL2Levels(
	...
	for (const level of levels) {
		const price = standardizePrice(level.price, grouping, side);
		const size = level.size;
		if (
...
export function uncrossL2(
	...
	const getPriceAndSetBound = (newPrice: BN, OrderSide: OrderSide) => {
		if (isVariant(OrderSide, 'buy')) {
	...
	while (bidIndex < bids.length || askIndex < asks.length) {
		...
		if (nextBid.price.gte(nextAsk.price)) {
			if (
				...
				newBidPrice = getPriceAndSetBound(newBidPrice, OrderSide.BUY);
				...
				newAskPrice = getPriceAndSetBound(newAskPrice, OrderSide.SELL);
			...
			if (minAsk && nextAsk.price.lte(minAsk)) {
				const newAskPrice = getPriceAndSetBound(nextAsk.price, OrderSide.SELL);
				updateLevels(newAskPrice, nextAsk, newAsks);
			} else {
				newAsks.push(nextAsk);
			}
			...
			if (maxBid && nextBid.price.gte(maxBid)) {
				const newBidPrice = getPriceAndSetBound(nextBid.price, OrderSide.BUY);
				updateLevels(newBidPrice, nextBid, newBids);
			} else {
				newBids.push(nextBid);
			}
