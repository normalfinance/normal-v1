...
export class DLOB {
	...
	private init() {
		this.openOrders.set('synthetic', new Set<string>());
		this.orderLists.set('synthetic', new Map<number, MarketNodeLists>());
	}
	...
	public getListForOrder(
		...
		if (isInactiveTriggerOrder) {
			...
			subType = isVariant(order.side, 'buy') ? 'bid' : 'ask';
		}
	...
	public updateRestingLimitOrders(slot: number): void {
		...
		this.updateRestingLimitOrdersForMarketType(slot, 'synthetic');
	}
	...
	public findNodesToFill(
		...
		const minAuctionDuration = isVariant(marketType, 'perp')
			? stateAccount.minAuctionDuration
			: 0;
	...
	getMakerRebate(
		...
		makerRebateNumerator =
			stateAccount.feeStructure.feeTiers[0].makerRebateNumerator;
		makerRebateDenominator =
			stateAccount.feeStructure.feeTiers[0].makerRebateDenominator;
	...
	*getBids(
	...
	public *getStopLosses(
		...
		if (isVariant(side, 'long') && marketNodeLists.trigger.below) {
			for (const node of marketNodeLists.trigger.below.getGenerator()) {
				if (isVariant(node.order.side, 'short')) {
					yield node;
				}
			}
		} else if (isVariant(side, 'short') && marketNodeLists.trigger.above) {
			for (const node of marketNodeLists.trigger.above.getGenerator()) {
				if (isVariant(node.order.side, 'long')) {
					yield node;
				}
			}
		}
	...
	public *getStopLossMarkets(
		...
		for (const node of this.getStopLosses(marketIndex, marketType, side)) {
	...
	public *getTakeProfits(
		...
		if (isVariant(side, 'buy') && marketNodeLists.trigger.above) {
			for (const node of marketNodeLists.trigger.above.getGenerator()) {
				if (isVariant(node.order.side, 'sell')) {
					yield node;
				}
			}
		} else if (isVariant(side, 'sell') && marketNodeLists.trigger.below) {
			for (const node of marketNodeLists.trigger.below.getGenerator()) {
				if (isVariant(node.order.side, 'buy')) {
					yield node;
				}
			}
		}
	...
	public *getTakeProfitMarkets(
		...
		for (const node of this.getTakeProfits(marketIndex, marketType, side)) {
			if (isVariant(node.order.orderType, 'triggerMarket')) {
				yield node;
			}
		}
	...
	public printTop(
		...
		if (isVariant(marketType, 'perp')) {
			...
			const name = decodeName(driftClient.getMarketAccount(marketIndex).name);
	...
	*getNodeLists(): Generator<NodeList<DLOBNodeType>> {
		for (const [_, nodeLists] of this.orderLists.get('synthetic')) {
	...
	public estimateFillWithExactBaseAmount({
		...
		if (isVariant(orderDirection, 'buy')) {
		...
		} else if (isVariant(orderDirection, 'sell')) {
			return this.estimateFillExactBaseAmountInForSide(
				baseAmount,
				oraclePriceData,
				slot,
	...
	public getBestMakers({
		...
		const generator = isVariant(side, 'long')
			? this.getRestingLimitBids(marketIndex, slot, marketType, oraclePriceData)
