...
export class PriorityFeeSubscriber {
	...
	public constructor(config: PriorityFeeSubscriberConfig) {
		...
		if (config.priorityFeeMethod) {
			...
			} else if (this.priorityFeeMethod === PriorityFeeMethod.NORMAL) {
	...
	private async loadForNormal(): Promise<void> {
		if (!this.normalMarkets) {
			return;
		}
		const sample = await fetchNormalPriorityFee(
