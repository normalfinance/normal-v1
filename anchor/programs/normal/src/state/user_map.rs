...
impl<'a> UserMap<'a> {
	#[track_caller]
	#[inline(always)]
	pub fn get_ref(&self, user: &Pubkey) -> NormalResult<Ref<User>> {
	...
	#[track_caller]
	#[inline(always)]
	pub fn get_ref_mut(&self, user: &Pubkey) -> NormalResult<RefMut<User>> {
		let loader = match self.0.get(user) {
			Some(loader) => loader,
			None => {
				let caller = Location::caller();
...
impl<'a> UserMap<'a> {
	pub fn load_one<'b: 'a>(
		...
		let user_account_loader: AccountLoader<User> = AccountLoader::try_from(
			account_info
...
impl<'a> UserStatsMap<'a> {
	#[track_caller]
	#[inline(always)]
	pub fn get_ref(&self, authority: &Pubkey) -> NormalResult<Ref<UserStats>> {
...
impl<'a> UserStatsMap<'a> {
	pub fn load_one<'b: 'a>(
		...
		let user_stats_account_loader: AccountLoader<UserStats> =
			AccountLoader::try_from(account_info).or(
				Err(ErrorCode::InvalidUserStatsAccount)
			)?;

		user_stats_map.0.insert(authority, user_stats_account_loader);
...
pub fn load_user_maps<'a: 'b, 'b>(
...
) -> NormalResult<(UserMap<'b>, UserStatsMap<'b>)> {
	...
	while let Some(user_account_info) = account_info_iter.peek() {
		...
		let user_stats_account_loader: AccountLoader<UserStats> =
			AccountLoader::try_from(user_stats_account_info).or(
				Err(ErrorCode::InvalidUserStatsAccount)
			)?;
...
pub fn load_user_map<'a: 'b, 'b>(
	...
	while let Some(user_account_info) = account_info_iter.peek() {
		...
		if
			data.len() < expected_user_data_len &&
			data.len() < expected_user_stats_len
		{
			break;
		}
